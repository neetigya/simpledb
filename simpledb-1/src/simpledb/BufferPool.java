...
public class BufferPool {
	...
	/**
	 * Retrieve the specified page with the associated permissions. Will acquire a lock 
	 * and may block if that lock is
	 * held by another transaction.
	 * <p>
	 * The retrieved page should be looked up in the buffer pool.
	 *  If it is present, it should be returned. If it is not
	 * present, it should be added to the buffer pool and returned.
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm) throws TransactionAbortedException,
		...
		Catalog cc = Database.getCatalog();
		Page pg = cc.getDbFile(pid.getTableId()).readPage(pid);
		if((pg= pages.get(pid))==null){
